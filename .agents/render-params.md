...
| `gradHeight` | `gradientHeight` | `qGradHeight` — alimenta l'altezza del gradiente/sfocatura (blurHeight; default 30 portrait, 20 landscape solo da query/config assenti) |
| `bf` | `blurFade` (slider editor 0..100 + double-click reset 50, 70 in landscape) | punto di attacco transizione 0..100 (default 50 portrait, 70 landscape): query > mapping > config token > server defaults > default di formato. Emessa sempre esplicita in preview e Stremio |
...
| `rw` | `defaultRatingPreset` (select Impostazioni, sempre esplicito in preview quando ≠ balanced) | `q.get("rw")` > server defaults (`PICTORIUM_RATING_PRESET`) > `"balanced"` — preset pesi voto (`balanced`/`cinephile`/`series`/`raw`); con `rsrc` esplicito filtra le fonti preset (intersezione vuota o nessun valore → fallback media pari legacy, mai badge vuoto); emesso negli URL Stremio solo quando ≠ `balanced` |
...
| `ac` | `accentColor` (auto: `findSceneTint` bottom-40% via `useRootColors`, come il server) | `qAc` — override colore accent (vince su tutto: badge colored, blur tint, scrim) |
