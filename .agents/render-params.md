...
| Testo pill/bar | bar = ad alto contrasto `bottomLight ? "rgba(255,255,255,0.95)" : "rgba(0,0,0,0.88)"` (su fondi chiari la barra diventa grafite scura, come la pill); pill genere = adattivo `bottomLight ? "rgba(255,255,255,0.85)" : "rgba(0,0,0,0.80)"`; pill ranking/extra = adattivo su `topLight` (pill colored resta tinta piatta + `textColorForBg`) |
...
| Ombra | `default`/extra-default/`pill` ranking-extra: ombra 3D singola stile nastro (`dx=3, dy=3, blur 3.5, 0.65`, solo contenitore) + padding `TOP_SHADOW_PAD=14` su lati/basso per non tagliare la coda nel viewport (in alto la placca resta a filo, altrimenti sembra staccata); `badgeShadowBox` resta per stime overflow — `blur = max(round(boxH * 0.20), 4)`, `off = max(round(boxH * 0.10), 2)` dove ancora usato |
| Sfondo | `default`/extra-default/`pill` — gradiente satinato `satinPillStops(topLight)` + bordo sagomato polarizzato 1.5px (`topLight ? "rgba(0,0,0,0.12)" : "rgba(255,255,255,0.22)"`) + ombra 3D singola sul contenitore, canvas = box + padding ombra (`totalW + 28 × boxH + 14`, filo in alto); `netflix` — nastro invariato (con ombra 3D propria); `colored` — tinta accent piatta + `textColorForBg` (lo stile `bar` del ranking è rimosso) |
